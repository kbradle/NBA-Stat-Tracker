...
function test(firstTeam, secondTeam, groupBy, allOthers, startDate, endDate){
    ...
    if(groupBy==='month'){        
        var data = "query= select t.team_name, sum(d.threepoints_attempted) as \"3PTA\",  Extract(year from g.game_date)as gyear, Extract(month from g.game_date)as gmonth from jawatson.games_details d, jawatson.teams t, JAWATSON.games g where d.team_id = t.team_id and g.game_id = d.game_id and (t.team_name = '"+firstTeam +"' or t.team_name = '"+secondTeam+"') and (g.game_date >= to_date('"+startDate+"', 'YYYY-MM-DD') and g.game_date <= to_date('"+endDate+"', 'YYYY-MM-DD')) group by t.team_name, Extract(year from g.game_date), Extract(month from g.game_date) order by Extract(year from g.game_date), Extract(month from g.game_date) asc";
        xhr.send(data);
        ...
        if(allOthers==='true'){
            ...
            if(secondTeam !== undefined)
                count++;
            var temp = "query= select sum(d.threepoints_attempted)/"+(30 - count)+" as \"3PTA\", Extract(year from g.game_date)as gyear, Extract(month from g.game_date)as gmonth from jawatson.games_details d, jawatson.teams t, JAWATSON.games g where d.team_id = t.team_id and g.game_id = d.game_id and t.team_name != '"+firstTeam +"' and t.team_name != '"+secondTeam+"' and (g.game_date >= to_date('"+startDate+"', 'YYYY-MM-DD') and g.game_date <= to_date('"+endDate+"', 'YYYY-MM-DD')) group by Extract(year from g.game_date), Extract(month from g.game_date) order by Extract(year from g.game_date), Extract(month from g.game_date) asc";
            xhrAll.send(temp);
        }
    }
    else{        
        var data1 = "query= select t.team_name, sum(d.threepoints_attempted) as \"3PTA\",  Extract(year from g.game_date)as gyear from jawatson.games_details d, jawatson.teams t, JAWATSON.games g where d.team_id = t.team_id and g.game_id = d.game_id and (t.team_name = '"+firstTeam +"' or t.team_name = '"+secondTeam+"') and (g.game_date >= to_date('"+startDate+"', 'YYYY-MM-DD') and g.game_date <= to_date('"+endDate+"', 'YYYY-MM-DD')) group by t.team_name, Extract(year from g.game_date) order by Extract(year from g.game_date) asc";
        xhr.send(data1);
        ...
        if(allOthers==='true'){
            ...
            if(secondTeam !== undefined)
                count1++;
            var temp1 = "query= select sum(d.threepoints_attempted)/"+(30 - count1)+" as \"3PTA\", Extract(year from g.game_date)as gyear from jawatson.games_details d, jawatson.teams t, JAWATSON.games g where d.team_id = t.team_id and g.game_id = d.game_id and t.team_name != '"+firstTeam +"' and t.team_name != '"+secondTeam+"' and (g.game_date >= to_date('"+startDate+"', 'YYYY-MM-DD') and g.game_date <= to_date('"+endDate+"', 'YYYY-MM-DD')) group by Extract(year from g.game_date) order by Extract(year from g.game_date) asc";
            xhrAll.send(temp1);
        }
    }
    ...
    xhrAll.onload = function() {
        if (xhrAll.status !== 200) { 
            ...
            if(groupBy === 'month'){
                for(var i=0; i<obj.message.rows.length;i++){
                        others.push({y: obj.message.rows[i][0], label: monthNames[obj.message.rows[i][2]-1] + ' '+ obj.message.rows[i][1]});                  
                }   
            }
            else{
                for(i=0; i<obj.message.rows.length;i++){
                        others.push({y: obj.message.rows[i][0], label: obj.message.rows[i][1]});
                }    
            }
    ...
    render(){
        ...
        return(
            ...
            <form onSubmit={this.mySubmitHandler}>
                ...
                <div >             
                    <input type="radio" value="true" checked={this.state.allOthers === 'true'} onChange={this.ChangeAll}/>All Other Teams On  
                    <input type="radio" value="false" checked={this.state.allOthers === 'false'} onChange={this.ChangeAll}/>All Other Teams Off          
                </div>
